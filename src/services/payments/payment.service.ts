...
export async function getEnabledGateways(): Promise<{
  ...
  const stripe = gateways.some(g => g.provider === 'stripe');
  const paypal = gateways.some(g => g.provider === 'paypal');
  const square = gateways.some(g => g.provider === 'square');
  const braintree = gateways.some(g => g.provider === 'braintree');
  const authorize = gateways.some(g => g.provider === 'authorize');
