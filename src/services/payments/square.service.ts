...
async function getSquareConfig() {
  const settings = await prisma.paymentGateway.findFirst({
  ...
  return {
    accessToken: settings.secretKey,
    applicationId: settings.publicKey,
    locationId: settings.merchantId,
    sandbox: settings.testMode,
    webhookSignature: settings.webhookSecret
  };
}
