  ...
  try {
    ...
    const optionsSvg = options.map((option, index) => {
      ...
      return `
        ...
        <rect x="${padding}" y="${y}" width="${Math.max(barWidth, 0)}" height="${optionHeight}" rx="8" fill="#0d7a3e" opacity="0.8"/>
        ...
        <text x="${width - padding - 16}" y="${y + 32}" font-family="Arial, sans-serif" font-size="20" fill="#0d7a3e" font-weight="700" text-anchor="end">${percentage}%</text>
      `;
    }).join('');
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Logo/Brand -->
  <text x="${padding}" y="${padding + 30}" font-family="Arial, sans-serif" font-size="24" fill="#0d7a3e" font-weight="700">${businessName}</text>

  <!-- Poll indicator -->
  <rect x="${padding + 140}" y="${padding + 12}" width="60" height="28" rx="14" fill="#dcfce7"/>
  <text x="${padding + 170}" y="${padding + 31}" font-family="Arial, sans-serif" font-size="14" fill="#0d7a3e" text-anchor="middle" font-weight="600">POLL</text>
